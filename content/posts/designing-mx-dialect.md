...
Block scaling fixes this by using many scales instead of one. Split the data into fixed-size blocks — 32 contiguous elements, in MX — and give each block its own scale. A value is reconstructed as `mantissa × block_scale`:
...
A note on scope: this is v1, a deliberately minimal first version — four ops and one type, with some features held back to later versions (marked v1.5 or v2 as they come up).
...
Three of the four are shape-and-type contracts an interface could almost describe on its own. `block_matmul` is the exception, and one detail of its signature will matter later: it takes the accumulator `C` as an *operand*, not just a result. It computes `C += A · B`, reading the incoming `C` and adding to it, the way a GEMM (General Matrix Multiply) does. That looks like a modeling nicety. It's the second of the two co-designed decisions, and like the first, the thing that justifies it doesn't show up until much later: at bufferization, not lowering.
...
The block axis was a choice at the type stage, forced by the lowering stage. The second decision runs the other way: a choice in an op's signature, forced by bufferization.

## The accumulator operand

The second cross-stage force sits one stage further down. The first was a type-stage choice forced by the lowering stage. This one is an op-signature choice forced by bufferization, near the end of the pipeline.
...
v1 is four ops and one type, and the boundaries are deliberate. The things left out are the things that would have grown the scope past what one version can defend: a quantized `B` operand (v1 leaves `B` in `f32`), per-channel scales, group quantization across dimension boundaries, and a real lowering for `fold_scale` (v1 only cleans it up, never lowers it). Each is noted where it comes up; none is an accident.

The rest of the series follows the pipeline down, one stage per post:
...
- **Post 5** drives the whole thing to running code, hits two real upstream limits, and measures the memory traffic the dialect was built to cut.

The thesis was that a dialect is a set of co-designed decisions, not a parts catalog. The two moments in this post were the evidence: a type that blocks its last axis because a matmul later in the pipeline reduces along it, and an op that carries an accumulator because bufferization at the end of the pipeline needs a destination to write into. The ops are the easy part. What makes it a dialect is that the decisions behind them are wired to each other, across the whole pipeline.